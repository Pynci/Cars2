...
public class BlueCarAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // Reward personalizzata per BlueCar
        AddReward(0.02f); // base reward pi� aggressiva
    }

    public override void Heuristic(in ActionBuffers actionsOut)
    {
        var c = actionsOut.ContinuousActions;
        c[0] = Input.GetKey(KeyCode.UpArrow) ? 1f : 0f;
        c[1] = Input.GetKey(KeyCode.LeftArrow) ? -1f : Input.GetKey(KeyCode.RightArrow) ? 1f : 0f;
        c[2] = Input.GetKey(KeyCode.DownArrow) ? 1f : 0f;
    }
}
